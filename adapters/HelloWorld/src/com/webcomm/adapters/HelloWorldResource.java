...
public class HelloWorldResource {
	...
	static Logger logger = Logger.getLogger(HelloWorldResource.class.getName());

    //Define the server api to be able to perform server operations
    WLServerAPI api = WLServerAPIProvider.getWLServerAPI();

	/* Path for method: "<server address>/SKHAdapter/adapters/HelloWorld/users" */
	@GET
	...
	/* Path for method: "<server address>/SKHAdapter/adapters/HelloWorld/users/{username}" */
	@GET
	@Path("/{username}")
	...
	/* Path for method: "<server address>/SKHAdapter/adapters/HelloWorld/users/helloUserQuery?name=value" */
	@GET
	@Path("/helloUserQuery")
	...
	/* Path for method: "<server address>/SKHAdapter/adapters/HelloWorld/prop" */
	@GET
	@Path("/prop")
	...
	/* Path for method: "<server address>/SKHAdapter/adapters/HelloWorld/users/{first}/{middle}/{last}?age=value" */
	@POST
	...
	public String enterInfo(@PathParam("first") String first, @PathParam("middle") String middle, @PathParam("last") String last,
			@QueryParam("age") int age, @FormParam("height") String height, @HeaderParam("Date1") String date){
		return first +" "+ middle + " " + last + "</br>" +
				"Age: " + age + "</br>" +
				"Height: " + height + "</br>" +
				"Date: " + date;
	}
	
	/* Path for method: "<server address>/SKHAdapter/adapters/HelloWorld/users/newUsers" */
	@PUT
	@Path("/newUsers")
